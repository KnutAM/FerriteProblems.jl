...
# the `assemble_cell!` in the original example, but we don't have to reinit! the 
# cellvalues as this has already been done. 
...
# At this point, we are now ready to define a problem that can be solved 
# as `problem = FerriteProblem(setup_problem_definition())`. 
# However, if we want to have any results, apart from the final state,
# we need to define postprocessing tasks. In contrast to the original example, 
...
# Wrap solver to make it save any open stuff at the end, 
# also in case of an exception 
# This is temporary, would be nice to handle this in FESolvers
